...
data_prep_model_ITS <- function(df){
  df1 <- df%>%
    arrange(monthdate) %>%
  ...
  ### Call model in jags
  source('./R/ITS_jags.R')
  posterior_samples<-ITS_model(burnin = burnin, samples=samples,thin=1,chains=1,n_full=n,n_modeling=n_pre,y=y,offset=offset,z=z)
  ...
  ### Print output
  output.list<-list('rr.samples'=rr_combined,'rr.q'=rr.q,"lambda_counter"=lambda_counter,"lambda"=lambda)

...
x.preds <- x.scale1[,1:ncol(x.scale1)] ## change to 2 if include offset as denom
...
### Call model in jags
source('./R/SC_ridge_noAR1.R')

mod_test<-reference(burnin = burnin, samples=samples,thin=1,chains=1,n_full=n,n_modeling=n_pre,y=y,offset=offset,x=x.preds,z=z)

### Post-processing
posterior_samples.all<-do.call(rbind,mod_test)
y_pred <- posterior_samples.all[,grep('Y_pred', dimnames(posterior_samples.all)[[2]])]
thetas <- posterior_samples.all[,grep('thetas', dimnames(posterior_samples.all)[[2]])]
...
### Print output
output.list<-list('rr.samples'=rr.agg,'rr.q'=rr.q,"y.pred"=y_pred,"y_fitted"=y_fitted,'log.rr.pointwise.q'=log.rr.pointwise,'thetas'=thetas)
